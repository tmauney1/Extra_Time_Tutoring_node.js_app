...
function populateTable() {
  ...
  $.getJSON( '/users/studentlist', function( data ) {
    ...
    $.each(data, function(){
      tableContent += '<tr>';
      tableContent += '<td><a href="#" class="linkshowuser" rel="' + this.studentname + '">' + this.studentname + '</a></td>';
...
function showUserInfo(event) {
  ...
  //Populate Info Box
  $('#studentInfoName').text(thisStudentObject.parentname);
  $('#studentInfoAge').text(thisStudentObject.age);
  $('#studentInfoGender').text(thisStudentObject.gender);
  $('#studentInfoLocation').text(thisStudentObject.location);
...
function addUser(event) {
  ...
  if(errorCount === 0) {
    ...
    var newUser = {
      'studentname': $('#addUser fieldset input#inputStudentName').val(),
      'subject': $('#addUser fieldset input#inputSubject').val(),
	  'time': $('#addUser fieldset input#inputTime').val(),
